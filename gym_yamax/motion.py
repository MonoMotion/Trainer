...
class EffectorTarget(object):
    ...
    def interp(self, t, t1, t2, target):
        if self.location is not None:
            location = self.location.interp(t, t1, t2, target.location)
        else:
            assert target.location is None
            location = None

        if self.rotation is not None:
            rotation = self.rotation.interp(t, t1, t2, target.rotation)
        else:
...
def get_frame_at(t, motion_iter):
    ...
    def calc_gap(t, t1, t2, p1, p2, e1, e2):
        ...
        def conv(eff):
            return {l: make_effector_target(e) for l, e in eff.items()}
